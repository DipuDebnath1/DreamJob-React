...
const JobCard = ({jobData}) => {
    // console.log(jobData);
    const{ id,logo,job_title,company_name,location,salary,job_type,remote_or_onsite} = jobData;

    return (
        <div className="flex flex-col gap-5 p-8 border rounded">
            <figure><img className="h-24" src={logo} alt="" /></figure>
            <h3 className="text-xl font-bold">{job_title}</h3>
            <p className="font-bold underline">{company_name}</p>
            <div>
                <button className="bg-indigo-100 py-2 px-3 rounded">{remote_or_onsite}</button>
                <button className="bg-indigo-100 py-2 px-3 rounded ml-5">{job_type}</button>
            </div>
